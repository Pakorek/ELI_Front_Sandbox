  ...
  questions: [
    {
      id: 1,
      label: "New Question",
      answers: [{ id: 1, label: "New Answer...", questionID: 1, isCorrect: false }],
      uniqueAnswer: true,
    },
  ],
};
...
export function QuizEditor(): JSX.Element {
  ...
  return (
    <QuizContext.Provider value={dispatch}>
      <Container>
        <div
          style={{ width: "50%", padding: "24px", border: "1px solid black" }}
        >
          <h2>{state.title}</h2>
        ...
        >
          ...
          <form onSubmit={formSubmission} noValidate>
            ...
            {state.questions.map((q: Question, key: number) => (
              <CreateQuestion dispatch={dispatch} question={q} key={key} />
            ))}
